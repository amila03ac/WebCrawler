...
POSITIVE = re.compile(".*post.*|.*hentry.*|.*entry.*|.*content.*|.*txt.*|.*body.*")
BR = re.compile("<br */? *>[ \r\n]*<br */? *>")
...
def simplify_html_before(soup):
    ...
    comments = soup.findAll(txt=lambda txt:isinstance(txt, Comment))
    [comment.extract() for comment in comments]

    # you can add more rules here!

    map(lambda x: x.replaceWith(x.text.strip()), soup.findAll("li"))    # tag to txt
    map(lambda x: x.replaceWith(x.text.strip()), soup.findAll("em"))    # tag to txt
    map(lambda x: x.replaceWith(x.text.strip()), soup.findAll("tt"))    # tag to txt
    map(lambda x: x.replaceWith(x.text.strip()), soup.findAll("b"))     # tag to txt
